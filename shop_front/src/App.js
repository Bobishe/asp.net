...
function App() {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(checkApiAvailability, 60000);
  ...
  return (
    <div>
      ...
      <Router>
        <div className='header'>
          <div className='header_content'>
            <ul>
              <li><Link to="/" className='header_link'>Главная</Link></li>
              <li><Link to="/catalog" className='header_link'>Каталог</Link></li>
              {isLoggedIn ? (
                <>
                  <li><Link to="/admin" className='header_link'>admin</Link></li>
                  <li><button onClick={handleLogout}>Выйти</button></li>
                </>
              ) : (
                <li><Link to="/login" className='header_link'>Войти</Link></li>
              )}
            </ul>
          </div>
        </div>
      ...
      {!apiAvailable && (
        <div className="alert_stop">API доступен только с 8:00 до 20:00.</div>
      )}
